...
- **Light and color** — exposure, contrast, highlights, shadows, whites, blacks,
  ...
  color mixer
- **Detail** — clarity, sharpening, noise reduction, vignette
...
- **Local adjustments** — radial, linear, and subject masks, each with their own
  tonal controls
- **Crop and straighten** — aspect presets, an arc ruler that follows your finger,
  and a one-click headshot crop that finds the face
- **Auto** — a single pass that sets exposure, white balance, and contrast from
...
| `set_edit` | Change any adjustment; renders live, takes an `intent` string shown in the UI |
...
worth keeping, match a look across fifty photos, or read the histogram back and
...
Bundled fonts — Fraunces, Geist, and Geist Mono — are used under the SIL Open
