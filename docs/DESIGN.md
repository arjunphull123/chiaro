...
- **Every version asks for permission again.** Releases are ad-hoc signed
  (ADR 0014), so the signing identity is that build's hash and macOS treats
  each update as a new app: the Gatekeeper approval is asked for again (even
  on `brew upgrade`, which carries approval forward only when the identity is
  unchanged), and so is access to Documents, Desktop or a card. A Developer ID
  certificate is the only fix. Building from source avoids Gatekeeper
  entirely, since nothing compiled locally is quarantined.
